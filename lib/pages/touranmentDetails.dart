...
class _TournamentDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final BoxDecoration underlineDecoration = new BoxDecoration(border: new Border(bottom: new BorderSide(color: themeData.accentColor)));
...
class _ContactItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ThemeData themeData = Theme.of(context);
    final List<Widget> columnChildren = lines.sublist(0, lines.length ).map((String line) => new Text(line)).toList();
          ...
          slivers: <Widget>[
                    ...
                    const DecoratedBox(
                      decoration: const BoxDecoration(
                        gradient: const LinearGradient(
                          ...
                          colors: const <Color>[const Color(0x60000000), const Color(0x0000000)],
                        )
                      ),
                    )
                  ],
            ...
            new SliverList(
              delegate: new SliverChildListDelegate(<Widget>[
                new _TournamentDetails(icon: Icons.location_on,
                children: <Widget>[new _ContactItem(lines: ['Location of Tournament'])],)
              ]
              ),
            ),
          ],
